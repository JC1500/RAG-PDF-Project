...
async def extract_memory(message: str, memory: Optional[list[str]] = None) -> None | list[str]:
    """
    ...
    system_prompt = """
            ...
            Existing memory(which can be empty) is given as: {memory}
    ...
    response = await extractor_model.ainvoke(msg)
...
if __name__ == '__main__':
    res = asyncio.run(extract_memory(message='i will call you tomorrow'))
    print(res)
    if res is not None:
        print(res)